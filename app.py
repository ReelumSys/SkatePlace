...
def create_map(lat, lon, zoom_level, spots, show_heatmap, show_markers):
    ...
    if show_markers:
        ...
        for spot in spots:
            ...
            folium_marker = folium.Marker(location=[spot['lat'], spot['lon']], popup=f"<b>{spot.get('name', 'Unbenannt')}</b>", tooltip=spot.get('name', 'Spot'), icon=folium.Icon(color=color, icon=type_icons.get(spot.get('type', 'Andere'), 'info-sign')))
            folium_marker.add_to(marker_cluster)
